...
checkpoint_dir = './checkpointsConvNeXt/'
history_file = 'historyConvNeXt.txt'
split_info_path = './split_info_ConvNeXt.pth'

# Initialisiere das Modell (ConvNeXt-Tiny)
def initialize_model():
    # Wähle das ConvNeXt-Tiny-Modell
    model = models.convnext_tiny(weights='DEFAULT')

    # Erhalte die Anzahl der Eingangsmerkmale für den Fully Connected Layer
    num_features = model.classifier[2].in_features
    model.classifier[2] = nn.Linear(num_features, 2)  # 2 Klassen (Pneumonia / Normal)
