#pip install torch torchvision tqdm timm transformers
...
class SegFormer(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # Load SegFormer pre-trained backbone from HuggingFace or timm
        self.segformer = SegformerForSemanticSegmentation.from_pretrained(
            "nvidia/segformer-b0-finetuned-ade-512-512", 
            num_labels=num_classes,  # Nur 1 Klasse für binäre Segmentierung
            ignore_mismatched_sizes=True  # Ignoriere Größenfehler beim Laden
        )

    def forward(self, x):
        # SegFormer expects input in the form of [batch, channels, height, width]
        return self.segformer(x).logits  # Output logits (without sigmoid)
...
def train(model, device, train_loader, val_loader, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        save_model(epoch, model, optimizer, scheduler, val_loss, f'model_segformer_epoch{epoch+1}_step2.pth')
        if epoch != 0: os.remove(f'model_atun_epoch{epoch}.pth')
        
        # Save the best model
        if val_loss < best_loss:
            best_loss = val_loss
            save_model(epoch, model, optimizer, scheduler, val_loss, 'best_model_segformer.pth')
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((512, 512)),  # Resizing to match SegFormer input size
        transforms.ToTensor()
    ])
